...
因此数据目录下出现同名文件会直接报错。只有分钟行情（`market/bars/1m/`）例外：
它是一天一个长表文件，本来就是要带进本地还没有的文件名，所以按文件名里的日期匹配，
不进入这份同名索引。

交付目录里的三个压缩包各自有不同的更新语义：
...
**`Kline_incre.zip`（按交易日整文件新增）**
一层扁平结构，成员是 `kline_day_YYYYMMDD.pkl`，每个是一天的分钟长表（一只股票
一分钟一行），对应 `data/full/market/bars/1m/`。这里的单位是「天」而不是「格子」：
分钟文件没有可合并的日期轴，本地没有的交易日按交付原样落地，本地已有的交易日只做
比对，不覆盖。

- 新增的交易日会检查字段齐全（`code`、`date`、`time` 和 6 个行情字段）、内容非空、
  文件里只有一个日期且与文件名一致、`code`/`time` 不重复。任一条不满足记 `ERROR`，
  只作废这一天，其余交易日继续处理。
- 本地已有的交易日：长表的行序不带信息，因此先按 `code`/`time` 排序再逐字段比较。
  一致就跳过（重复交付同一批是正常的）；不一致记 `ERROR`——增量应当原样重复历史，
  直接覆盖只会把「两份里哪一份是错的」这个问题埋掉。
- 成员名认不出交易日、同一个交易日出现两次、包里没有任何分钟文件，都说明交付形态
  变了，按该数据源整体失败处理。
- 最后以更新后的 `trd_cal.pkl` 为基准，检查「本地已有 + 本次新增」的交易日序列：
  空洞、多余的非交易日、超过 `MAX_DATE_LAG_DAYS` 的尾部滞后都记 `ERROR`。分钟行情
  一天一个文件，漏掉一天不会被任何矩阵的日期轴发现，只能从文件名上查出来。
